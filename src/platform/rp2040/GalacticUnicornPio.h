...
inline const uint16_t instructions[] = {
  pio_encode_out(pio_y, 8),                                      // 0: pixel count - 1
  pio_encode_out(pio_pins, 8),                                   // 1: row select
  uint16_t(pio_encode_out(pio_x, 1) | pio_encode_sideset_opt(1, 0) | pio_encode_delay(1)),
  pio_encode_set(pio_pins, 4),
  pio_encode_jmp_not_x(6),
  pio_encode_set(pio_pins, 5),
  uint16_t(pio_encode_nop() | pio_encode_sideset_opt(1, 1) | pio_encode_delay(2)),
  uint16_t(pio_encode_out(pio_x, 1) | pio_encode_sideset_opt(1, 0) | pio_encode_delay(1)),
  pio_encode_set(pio_pins, 4),
  pio_encode_jmp_not_x(11),
  pio_encode_set(pio_pins, 5),
  uint16_t(pio_encode_nop() | pio_encode_sideset_opt(1, 1) | pio_encode_delay(2)),
  uint16_t(pio_encode_out(pio_x, 1) | pio_encode_sideset_opt(1, 0) | pio_encode_delay(1)),
  pio_encode_set(pio_pins, 4),
  pio_encode_jmp_not_x(16),
  pio_encode_set(pio_pins, 5),
  uint16_t(pio_encode_out(pio_null, 5) | pio_encode_sideset_opt(1, 1) | pio_encode_delay(2)),
  pio_encode_jmp_y_dec(2),
  pio_encode_out(pio_null, 8),                                   // padding byte
  uint16_t(pio_encode_set(pio_pins, 6) | pio_encode_delay(5)),     // latch, blank
  pio_encode_set(pio_pins, 0),                                   // enable output
  pio_encode_out(pio_y, 32),                                     // binary-weighted dwell
  pio_encode_jmp_y_dec(22),
  pio_encode_set(pio_pins, 4),                                   // blank before next row
};
